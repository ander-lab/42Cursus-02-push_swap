...
/*   Updated: 2021/11/27 16:48:25 by ajimenez         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_add_splitted_args(t_stack **stack_a, t_stack **stack_b ,char **av, size_t aux, t_struct *ps)
{
	size_t	i;
	char	**num;
	...
	while (num[i])
	{
		ft_check_signs(av[aux], stack_a, stack_b, ps);
...
void	ft_add_stack_a(t_stack **stack_a, t_stack **stack_b, t_struct *ps, char **av)
{
	...
	while (av[aux])
	{ 
		if (ft_iter_str_bool(av[aux], &ft_isalpha) || ft_iter_str_bool(av[aux], &ft_isalpha))
		{
			ft_putstr_fd("Error\n", 1); 
		...
		if (ft_iter_str_bool(av[aux], &ft_isspace))
			ft_add_splitted_args(stack_a, stack_b, av, aux, ps);
		else
		{
			ft_check_signs(av[aux], stack_a, stack_b, ps);
